...
<%-- 
	...
	6,在div的合适位置添加 分页插件 html代码 : 
		...
		前台完成
后台:
	1,将SimplePage.java文件贴到项目bean包
	2,创建分页servlet page.action
	3,在page.action中需要从请求中获取用户当前点击的是第几页;注:这个参数是固定的
		String pageNo = request.getParameter("pageNo");
	4,查询得到所有数据
		List<Book> alllist = service.findAll();
	5,创建SimplePage对象,并且给setPage();setPageDataNum();setPageNo();
		SimplePage sp = new SimplePage();
	6,通过操作得到当前页 应该显示的数据
		List<Book> showPage =new ArrayList<Book>();
		for(int i = sp.getPageStart();i<=sp.getPageEnd();i++){
			showPage.add(list.get(i));
		}
	7,把固定数据放到request容器中 : key都是固定的
		request.setAttribute("list", showPage);
		request.setAttribute("simplePage", sp);
		request.setAttribute("pageCount", sp.getPageCount());
		request.setAttribute("pageNo", sp.getPageNo());
	8,内部跳转到show.jsp页面
